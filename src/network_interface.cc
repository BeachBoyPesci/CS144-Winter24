...
void NetworkInterface::send_datagram( const InternetDatagram& dgram, const Address& next_hop )
{
  ...
  if ( auto it = arp_map_.find( next_hop_ipv4 ); it != arp_map_.end() ) {
    if ( wait_list_.find( next_hop_ipv4 ) == wait_list_.end() )
      return;
    ARPMessage arp_req_ { .opcode = ARPMessage::OPCODE_REQUEST,
    ...
    transmit( { .header = header, .payload = serialize( arp_req_ ) } );
  } else {
...
void NetworkInterface::recv_frame( const EthernetFrame& frame )
{
  ...
  // 如果传入的帧是IPv4，将有效载荷解析为InternetDatagram，如果成功（parse()
  // 方法返回ParseResult::NoError），将结果数据报推送到datagrams_received_queue。
  // 如果传入的帧是ARP，将有效载荷解析为ARPMessage，如果成功，记住发送方的IP地址和以太网地址的映射关系，持续30秒（从请求和回复中学习映射）。此外，如果是询问我们IP地址的ARP请求，发送一个适当的ARP回复。
  ...
  // 当数据帧的协议是 IPv4 时，且解析成功（parse()返回值为 true），那么就把解析得到的 InternetDatagram
  // 数据报推入队列 datagrams_received_ 中。 若数据帧协议为 ARP，且解析成功，那么按照得到的 ARPMessage
  // 中的信息，分析是否是请求本接口的 IP地址和以太网地址的映射关系、或者是否是响应之前本接口发出的 ARP 请求。 如果是
  // ARP 请求，那么就组装对应的ARPMessage 并发送给请求发送方；如果是 ARP 响应，那么就将先前缓存的、现在能发送的
  // IP数据报全部发送出去。 注意这里无论是 ARP 请求还是 ARP 响应，只要数据帧解析成功，都要从中学习新的地址映射关系。
}
...
// 因为同一个IP地址可以对应多个报文，所以在收到一个ARP响应时，可以使用std::unordered_multimap::equal_range找出所有下一跳地址相同的IP报文，然后逐一转发即可。
