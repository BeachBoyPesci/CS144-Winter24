...
void NetworkInterface::send_datagram( const InternetDatagram& dgram, const Address& next_hop )
{
  ...
  if ( auto it = arp_map_.find( ip ); it == arp_map_.end() ) {
    ...
    transmit( { .header = eth_header, .payload = serialize( arp_request ) } );
    boardcast_waitlist_[ip].first.emplace_back( dgram );
  } else {
    EthernetHeader eth_header
      = { .dst = it->second.first, .src = this->ethernet_address_, .type = EthernetHeader::TYPE_IPv4 };
    transmit( { .header = eth_header, .payload = serialize( dgram ) } );
  }
}
    ...
    if ( parse( arp_msg, frame.payload ) ) {
      ...
      if ( arp_msg.opcode == ARPMessage::OPCODE_REQUEST
        ...
        EthernetHeader arp_eth_header
          = { .dst = sender_mac, .src = this->ethernet_address_, .type = EthernetHeader::TYPE_ARP };
        transmit( { .header = arp_eth_header, .payload = serialize( arp_reply ) } );
      } else {
        if ( boardcast_waitlist_.contains( sender_ip ) ) {
          for ( auto dgram : boardcast_waitlist_[sender_ip].first ) {
            EthernetHeader dgram_eth_header
              = { .dst = sender_mac, .src = this->ethernet_address_, .type = EthernetHeader::TYPE_IPv4 };
            transmit( { .header = dgram_eth_header, .payload = serialize( dgram ) } );
          }
